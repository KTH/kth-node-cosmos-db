...
var Azure = function(opts) {
  ...
  if (missing.length > 0) {
    throw {
      message: 'One or more of the required config options is missing, please add these to the conf object',
      fields: missing.join(', ')
    }
  }
...
module.exports.getClient = () => {
  if (_development(process.env)) {
    log.debug(`You are accessing the kth-node-cosmos-db client in development (Please add process.env.USE_COSMOS_DB='true' if you want to use any of its functions)`)
    return Promise.resolve({
      increaseCollectionThroughput: () => log.debug(`increaseCollectionThroughput is not implemented in development`),
      updateCollectionThroughput: () => log.debug(`updateCollectionThroughput is not implemented in development`),
      updateAllCollectionsThroughput: () => log.debug(`updateAllCollectionsThroughput is not implemented in development`),
      listCollectionsWithThroughput: () => log.debug(`listCollectionsWithThroughput is not implemented in development`),
      getCollectionThroughput: () => log.debug(`getCollectionThroughput is not implemented in development`)
    })
  }
