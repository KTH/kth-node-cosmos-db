...
const ensureValidity = require('./ensureValidity')
...
function _hoistClassDefinition() {
  const definition = class CosmosClientWrapper {
     ...
     * @param {boolean} [options.createCollectionWithMongoose]
     */
    constructor(options) {
    ...
    _setDefaults() {
      ...
      this.createCollectionWithMongoose = false
    }

    _checkOptions() {
      const schema = CosmosClientWrapper._getSchemaOfKnownOptions()
      const checkForInvalidKeys = true
      try {
        ensureValidity({ input: this._options, schema, checkForInvalidKeys })
      } catch (error) {
        error.message = `kth-node-cosmos-db: Invalid client options - ${error.message}`
        throw error
      }
    }

    static _getSchemaOfKnownOptions() {
      const required = true
      const schema = {
        host: { type: 'hostname', required },
        username: { type: 'string', required },
        password: { type: 'azureKey', required },
        db: { type: 'azureDatabaseName', required },
        collections: { type: CosmosClientWrapper._isCollectionType, required },

        defaultThroughput: { type: 'azureThroughput' },
        maxThroughput: { type: 'azureThroughput', required },
        throughputStepsize: { type: 'azureThroughputSteps' },
        batchSize: { type: 'number' },

        port: { type: 'port' },
        disableSslRejection: { type: 'boolean' },
        createCollectionWithMongoose: { type: 'boolean' }
      }
      return schema
    }

    static _isCollectionType(input) {
    ...
    async _initSingleContainer(database, collectionIndex) {
      ...
      const containerWillBeCreatedLater = this.createCollectionWithMongoose
     ...
     */
    createMongooseModel(collectionName, mongooseSchema) {
      // @TODO
      // _ensureValidMongooseCollectionName(collectionName)
      // _ensureValidMongooseSchema(mongooseSchema)
      ...
      if (!collectionIsKnown) {
        throw new Error(
          "kth-node-cosmos-db: Can't create Mongoose model without knowing collection " +
            '- Adapt your input to createClient()!'
        )
      }
      ...
      const containerDidNotExistedBefore =
        this.createCollectionWithMongoose && this.collections[index].prepared === false
