      ...
      if (readErr && readErr.code === 404) {
        client.createCollection(
          ...
          createErr => {
            ...
            log.info(
              `kth-node-cosmos-db Created collection ${collection.name} with throughput: ${throughput}`
            )
        ...
        log.info(`kth-node-cosmos-db Collection ${collection.name} does allready exist.`)
        resolve()
      }
    })
  })
