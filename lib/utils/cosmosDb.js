 ...
 *    Resolves with container matching the given name; or
 ...
 *    Resolves after the container's throughput was updated
 * @throws
 *    e.g. in case of invalid arguments
 */
async function setContainerThroughput(container, newThroughput) {
  ...
  await formerOffer.replace(newOfferDefinition)
...
function _ensureValidObject(input, description, className) {
  assert(input != null && typeof input === 'object', `Invalid ${description} (got ${typeof input})`)
  assert(
    input.constructor.name === className,
    `Invalid ${description} (got instance of ${input.constructor.name})`
  )
}

function _ensureValidPrimitive(input, description) {
  assert(input != null, `Invalid ${description} (got null)`)
  assert(
    typeof input !== 'object',
    `Invalid ${description} (got instance of ${input.constructor.name} ` +
